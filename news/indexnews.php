...
$SiteId=$_REQUEST["noic"];
//$SiteId="5f7ec3b288df0";
							...
							<div class="rs-toolbar-right" style="text-align: left;">
								<div class="toolbar-share-icon">
									<ul>
										...
										<?php
										if($PrimaryEmail){
											echo "<li><a href=\"#\" target=\"_blank\" title=\"email\"><i class=\"fa fa-envelope\"></i> ".$PrimaryEmail."</a></li>\n";
										}
										
										if($phone){
											echo "<li><a href=\"#\" target=\"_blank\" title=\"phone\"><i class=\"fa fa-phone\"></i> ".$phone."</a></li>\n";
										}
...
?>										</ul>
        ...
        <div id="rs-team-2" class="rs-team-2 team-page sec-spacer">
			<div class="container">
				<div>
					<form name="frmtahun" method="post" action="">
					<strong>Select Year</strong>&nbsp;&nbsp;
					<select name="txtyear" onChange="location.href='indexnews.php?noic=<?php echo $SiteId;?>&year='+document.frmtahun.txtyear.value;" style="font-size: 15px;padding: 5px 10px;width: 100px;">
<?php
$qryYearNews = mysqli_prepare($dbi, "select year(dateupdate) as tahun from news where division = 'news' and SiteId = ? group by year(dateupdate) order by year(dateupdate) desc");
mysqli_stmt_bind_param($qryYearNews, "s", $SiteId);
mysqli_stmt_execute($qryYearNews);
mysqli_stmt_bind_result($qryYearNews, $dataNews["tahun"]);
while (mysqli_stmt_fetch($qryYearNews)) {
	$tahun = $dataNews["tahun"];
...
?>
                ...
                <div class="gridFilter">
                    ...
                    <button data-filter=".Jan">JAN</button>
                    <button data-filter=".Feb">FEB</button>
                    <button data-filter=".Mar">MARCH</button>
                    <button data-filter=".Apr">APRIL</button>
                    <button data-filter=".May">MAY</button>
                    <button data-filter=".Jun">JUN</button>
                    <button data-filter=".Jul">JULY</button>
                    <button data-filter=".Aug">AUG</button>
                    <button data-filter=".Sept">SEPT</button>
                    <button data-filter=".Oct">OCT</button>
                    <button data-filter=".Nov">NOV</button>
...
//$selectedyear = date("Y");
//Count News --------------
$qryBilNews = mysqli_prepare($dbi, "SELECT count(*) as BilNews FROM news where division = 'news' and SiteId = ? and year(dateupdate)= ? ");
mysqli_stmt_bind_param($qryBilNews, "ss", $SiteId, $selectedyear);
mysqli_stmt_execute($qryBilNews);
mysqli_stmt_bind_result($qryBilNews, $bilNews);
mysqli_stmt_fetch($qryBilNews);
mysqli_stmt_close($qryBilNews);
					
//echo "bil events = ".$bilEvents;

if($bilNews!="0"){
	$qryNews = mysqli_prepare($dbi, "SELECT id, title, image, content, counter, dateupdate FROM news where division = 'news' and SiteId = ? and year(dateupdate)= ? order by startdate desc");
	mysqli_stmt_bind_param($qryNews, "ss", $SiteId, $selectedyear);

}else{
	
	$qryNews = mysqli_prepare($dbi, "SELECT id, title, image, content, counter, dateupdate FROM news where division = 'news' and SiteId = ? and year(dateupdate)= ? order by startdate desc");
	mysqli_stmt_bind_param($qryNews, "ss", $SiteId, $tahun);
}
...
mysqli_stmt_bind_result($qryNews, $dataNews["id"], $dataNews["title"], $dataNews["image"], $dataNews["content"], $dataNews["counter"], $dataNews["dateupdate"]);
$bilnews = 1;
while (mysqli_stmt_fetch($qryNews)) {
	...
	$img = $dataNews["image"];
	if ($img){
		$imgpath = "../Image/News/".$img;
	}else{
		$imgpath = "../upload/news-no-pic.jpg";
	}

	$content = $dataNews["content"];
	$counter = $dataNews["counter"];
	$startdate = $dataNews["dateupdate"];
	...
	if ($startdate <> ""){ 
		$hari=substr($startdate,8,2);
		$bulan=substr($startdate,5,2);
		$bulan=(int)$bulan;
	...
	echo "<div class=\"col-lg-4 col-md-6 col-xs-6 grid-item ".$months[$bulan]."\">\n";
	echo "  <div class=\"team-item\">\n";
	echo "	<div class=\"team-img\">\n";
	echo "		<a href=\"".$pautan."\"><img src=\"".$imgpath."\" alt=\"\" /></a>\n";
	echo "		<div class=\"social-icon\">\n";
	echo "			<a href=\"".$pautan."\"><i class=\"fa fa-link\"></i></a>\n";
	echo "		</div>\n";
	echo "	</div>\n";
	echo "	<div class=\"team-body\">\n";
	echo "		<a href=\"".$pautan."\"><h3 class=\"name\">".limit_text($title,7)."</h3></a>\n";
	echo "	</div>\n";
	echo "  </div>\n";						
	echo "</div>\n";
...
?>					
					<!--<div class="col-lg-3 col-md-6 col-xs-6 grid-item Jan">
		                <div class="team-item">
		                    <div class="team-img">
					...
					<div class="col-lg-3 col-md-6 col-xs-6 grid-item Feb">
		                <div class="team-item">
		                    <div class="team-img">
					...
					<div class="col-lg-3 col-md-6 col-xs-6 grid-item Mar">
		                <div class="team-item">
		                    <div class="team-img">
